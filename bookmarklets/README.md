...
`web-chat-downloader serve`(`https://127.0.0.1:8787`)로 그대로 POST한다.
...
1. 로컬에서 서버를 띄운다.
   ...
   ```
   기본으로 `https://127.0.0.1:8787`에서 대기하고, 저장 위치 기본값은
   `~/Desktop/Archive/web-chats`다(`--port`, `--into`로 바꿀 수 있음).
2. **최초 1회만**: 브라우저에서 `https://127.0.0.1:8787`를 직접 열어본다.
   자체서명 인증서라 "이 연결은 비공개 연결이 아닙니다" 같은 경고가 뜨는데,
   "고급" → "127.0.0.1(안전하지 않음)으로 이동" 순서로 진행하면 된다.
   이 과정을 한 번 거쳐야 브라우저가 이 인증서를 신뢰해서, 이후 북마클릿의
   POST가 mixed-content 없이 통과한다(북마클릿은 https 페이지에서 실행되므로
   대상도 https여야 한다).
3. 캡처하려는 서비스에 로그인한 브라우저 탭에서, 저장하려는 대화 페이지를 연다.
   - claude.ai: `https://claude.ai/chat/<uuid>`
   - ChatGPT: `https://chatgpt.com/c/<id>`
   - Gemini: 대화 페이지 아무 곳
4. 해당 서비스용 북마크를 클릭한다.
5. `저장됨: <sessionId>` alert가 뜨면 성공. 서버 콘솔에도
